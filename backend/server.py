...
async def student_dues(student_id: str, request: Request, current=Depends(get_current_user)):
    ...
    dues_installments = _pick_assignment_installments(assignments)
    ...
    due_till_date = round(max(expected_till_date - total_paid, 0.0), 2)
...
def _build_month_schedule(net_annual: float, payments: list, session: str,
                          ...
                          installments: Optional[list] = None) -> tuple:
    ...
    equal_monthly = round(net_annual / 12.0, 2) if net_annual > 0 else 0.0
...
async def student_fee_schedule(student_id: str, request: Request, current=Depends(get_current_user)):
    ...
    # ---- Plan-baked discounts (part of the plan; NO owner approval) ----
    assignment_installments = _pick_assignment_installments(assignments)
    ...
    if assignment_installments:
        ...
        base_installments = [{'month': m['month'], 'year': m['year'], 'amount': per, 'status': 'active'} for m in months]
    ...
    you_saved = round(concession + plan_discount_total, 2)
